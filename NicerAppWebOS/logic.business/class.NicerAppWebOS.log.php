...
class class_NicerAppWebOS_log {
    ...
    public function add ($entries) {
        ...
        if ($naLAN || (isset($naWebOS) && $naWebOS->showAllErrors)) {
            $fncn = '.../NicerAppWebOS/logic.business/class.NicerAppWebOS.log.php::public function add()';
            echo '<h2>'.$fncn.'</h2>'.PHP_EOL;
            echo '<link type="text/css" rel="StyleSheet" href="/NicerAppWebOS/errors.css?c='.date('Ymd_His').'">'.PHP_EOL;
            echo '<pre class="naWebOS_phpTrace">'.str_replace("\n",'<br/>'.PHP_EOL,$e->getTraceAsString()).'</pre>'.PHP_EOL;
            foreach ($entries as $entryIdx => $entry) {
                $it = $entries[$entryIdx]['ENTRY:add_PHPerror'];
                echo '<div class="addLogEntries" style="margin-left:20px;"><span class="addLogEntry_idx">'.$entryIdx.'</span><div class="addLogEntry_data">'.$it['html'].'</div></div>'.PHP_EOL;
                error_log ($it['txt']);
            }
            //die();
        } else {
