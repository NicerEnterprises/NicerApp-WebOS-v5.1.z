...
global $scriptName_SAG;
$scriptName_SAG = 'Sag.php';
...
class Sag {
  ...
  public function bulkDocs ($data) {
    ...
    return $this->procPacket ('POST', '/'.$this->db.'/_bulk_docs', $data);
  }
  
  public function getView ($designDocName, $viewName, $startkey, $endkey) {
    if (!is_string($startkey)) $startkey = json_encode($startkey);
    global $scriptName_SAG; $this->checkJSONencoding ($scriptName_SAG, 'getView()', $startkey);
    if (!is_string($endkey)) $endkey = json_encode($endkey);
    global $scriptName_SAG; $this->checkJSONencoding ($scriptName_SAG, 'getView()', $endkey);
    $url = '/'.$this->db.'/_design/'.$designDocName.'/_view/'.$viewName.'?startkey='.$startkey.'&endkey='.$endkey.'&descending=true';
    $url = mb_convert_encoding ($url, 'UTF-8', 'UTF-8');
    //var_dump ($this->pathPrefix.$url); exit();
    return $this->procPacket ('GET', $url);
  }
  
  public function checkJSONencoding ($scriptName, $functionName, $jsonEncodeResult) {
    if ($jsonEncodeResult===false) {
        echo '<p style="color:yellow;background:black;padding:3px;"><span style="color:white;background:blue;padding:1px;">'.$scriptName.'::'.$functionName.' : </span>'.json_last_error_msg().'</p>'.PHP_EOL;
    }
  }
