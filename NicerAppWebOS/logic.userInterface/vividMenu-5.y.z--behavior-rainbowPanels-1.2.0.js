...
//debugger;
    ...
    showMenuItem (t, it, dim /* dimensions */, evt /* event */) {
        ...
        if (it.parents && it.parents[0]) {
            ...
            var column = lidx;
            console.log ('it.label='+it.label+', row='+row+', column='+column);
    ...
    getDimensions (t, el, restrict) {
        ...
        dim = { // dimensions
            space2right :
                ...
                ? $(e).width()
                : Math.abs($(window).width() - ebcr.left - bws.left - bws.right - 20),
            space2left : Math.abs(ebcr.left),
            space2top : Math.abs(ebcr.top - bws.top),
            space2bottom : Math.abs($(window).height() - ebcr.top - $(e).height() - bws.top - bws.bottom),
        };
        if (el.it.numColumns) {
